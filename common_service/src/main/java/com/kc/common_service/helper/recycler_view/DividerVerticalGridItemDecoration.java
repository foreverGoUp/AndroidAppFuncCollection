package com.kc.common_service.helper.recycler_view;
 ...
 */
public class DividerVerticalGridItemDecoration extends RecyclerView.ItemDecoration
{
     ...
     * */
    public DividerVerticalGridItemDecoration(Context context, int drawableResId) {
     ...
     * */
    public DividerVerticalGridItemDecoration(Context context, int drawableResId, int dpOfHeightOfBottomShadowOfItem) {
    ...
    public void getItemOffsets(@NonNull Rect outRect, @NonNull View view, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if (isFirstColumn(parent, childCount, spanCount, itemPosition)){//如果是第一列，画item右边一半分割线和底部分割线。
            if (isLastRaw(parent, childCount, spanCount, itemPosition)){//如果是最后一行，则item底部不画分割线
                outRect.set(0, 0, mDivider.getIntrinsicWidth()/2, 0+mHeightOfBottomShadowOfItem);
            }else {
                outRect.set(0, 0, mDivider.getIntrinsicWidth()/2, mDivider.getIntrinsicHeight());
            }
        } else if (isLastColumn(parent, childCount, spanCount, itemPosition))// //如果是第一列，画item左边一半分割线和底部分割线。
        {
            if (isLastRaw(parent, childCount, spanCount, itemPosition)){//如果是最后一行，则item底部不画分割线
                outRect.set(mDivider.getIntrinsicWidth()/2, 0, 0, 0+mHeightOfBottomShadowOfItem);
            }else {
                outRect.set(mDivider.getIntrinsicWidth()/2, 0, 0, mDivider.getIntrinsicHeight());
            }
        } else//非第一列和非最后一列的，画item左右两边各一半分割线和底部分割线。
        {
            if (isLastRaw(parent, childCount, spanCount, itemPosition)){//如果是最后一行，则item底部不画分割线
                outRect.set(mDivider.getIntrinsicWidth()/2, 0, mDivider.getIntrinsicWidth()/2,
                        0+mHeightOfBottomShadowOfItem);
            }else {
                outRect.set(mDivider.getIntrinsicWidth()/2, 0, mDivider.getIntrinsicWidth()/2,
